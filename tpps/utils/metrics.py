...
from tpps.utils.history_bst import get_history_and_target_all, get_repeated_prev_times

def mark_metrics(pred, gold, probas, ranks, mask, results, n_class, multi_labels=False, test=False):
    if multi_labels:
...
def get_time_prediction(model, batch, args, alpha_val=0.5):
    ...
    low = valid_cdf > alpha_val - tolerance
    high = valid_cdf < alpha_val + tolerance
    valid = low * high
